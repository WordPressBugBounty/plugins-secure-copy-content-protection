...
class Ays_Sccp_Data {
    ...
    public static function ays_sccp_allowed_html() {
    ...
    public static function ays_sccp_custom_allowed_html() {
        $additionalAllowedTags = self::ays_sccp_allowed_html();
        return array_merge(
            $additionalAllowedTags,
            array(
                'iframe' => array(
    ...
    public static function ays_sccp_sale_baner(){
        ...
        if($ays_sccp_ishmar == 0){
            if (isset($_GET['page']) && strpos($_GET['page'], SCCP_NAME) !== false) {                
                ...
                // self::ays_sccp_footer_sale_banner_2026($ays_sccp_ishmar);
                   self::ays_sccp_back_to_school_banner_2026($ays_sccp_ishmar);
            }
        }
    } 
 ...
 // Back to School top banner
    public static function ays_sccp_back_to_school_banner_2026($ishmar){
        if($ishmar == 0 ){
            $content = array();
            $sccp_cta_button_link = esc_url('https://ays-pro.com/wordpress/secure-copy-content-protection?utm_source=dashboard&utm_medium=sccp-free&utm_campaign=back-to-school-sale-banner-' . SCCP_NAME_VERSION);
            $content[] = '<div id="ays-sccp-back-to-school-banner-2026" class="ays-sccp-back-to-school-banner-2026 ays-sccp-admin-notice notice notice-success is-dismissible ays_sccp_dicount_info">';
                $content[] = '<svg class="ays-sccp-bts-decoration ays-sccp-bts-plane" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 2 11 13"></path><path d="M22 2 15 22l-4-9-9-4Z"></path></svg>';
                $content[] = '<svg class="ays-sccp-bts-decoration ays-sccp-bts-ruler" viewBox="0 0 36 20" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" aria-hidden="true"><rect x="1" y="6" width="34" height="9" rx="2"></rect><path d="M8 6v3M14 6v4M20 6v3M26 6v4"></path></svg>';
                $content[] = '<svg class="ays-sccp-bts-decoration ays-sccp-bts-star" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="m12 2 2.6 6.6L21.5 9l-5 4.6 1.4 7-5.9-3.5L6.1 20.6l1.4-7L2.5 9l6.9-.4Z"></path></svg>';
                $content[] = '<svg class="ays-sccp-bts-decoration ays-sccp-bts-pencil" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"></path><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"></path></svg>';
                $content[] = '<svg class="ays-sccp-bts-decoration ays-sccp-bts-wave" viewBox="0 0 96 12" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" aria-hidden="true"><path d="M2 8c8-8 16 8 24 0s16 8 24 0 16 8 24 0 16 8 20 2"></path></svg>';
                $content[] = '<div class="ays-sccp-bts-content">';
                    $content[] = '<div class="ays-sccp-bts-offer">';
                        $content[] = '<img class="ays-sccp-bts-handwritten" src="'. esc_url(SCCP_ADMIN_URL . '/images/ays-sccp-back-to-school-handwritten.svg') .'" alt="Back to School Sale">';
                        $content[] = '<span class="ays-sccp-bts-dot" aria-hidden="true"></span>';
                        $content[] = '<span class="ays-sccp-bts-discount">'. esc_html__('20% OFF', 'secure-copy-content-protection') .'</span>';
                    $content[] = '</div>';
                    $content[] = '<div class="ays-sccp-bts-actions">';
                        $content[] = '<span class="ays-sccp-bts-use-code">'. esc_html__('Use code', 'secure-copy-content-protection') .'</span>';
                        $content[] = '<button type="button" class="ays-sccp-bts-coupon" data-coupon="SCHOOL20" aria-label="'. esc_attr__('Copy coupon code SCHOOL20', 'secure-copy-content-protection') .'"><span>SCHOOL20</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="14" height="14" x="8" y="8" rx="2"></rect><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"></path></svg></button>';
                        $content[] = '<a href="'. esc_url($sccp_cta_button_link) .'" class="ays-sccp-bts-cta" target="_blank" rel="noopener noreferrer">'. esc_html__('Upgrade Now', 'secure-copy-content-protection') .'</a>';
                    $content[] = '</div>';
                $content[] = '</div>';
                $content[] = '<div class="ays-sccp-bts-dismiss"><form action="" method="POST"><div id="ays-sccp-dismiss-buttons-content">';
                    if( current_user_can( 'manage_options' ) ){
                        $content[] = '<button class="btn btn-link ays-button" name="ays_sccp_sale_btn">'. esc_html__('Dismiss ad', 'secure-copy-content-protection') .'</button>';
                        $content[] = wp_nonce_field( SCCP_NAME . '-sale-banner', SCCP_NAME . '-sale-banner', true, false );
                    }
                $content[] = '</div></form></div>';
            $content[] = '</div>';
            $content[] = '<style id="ays-sccp-back-to-school-banner-2026-inline-css">';
            $content[] = '#ays-sccp-back-to-school-banner-2026{box-sizing:border-box;position:relative;isolation:isolate;width:calc(100% - 20px);max-width:none;min-height:112px;margin:20px 20px 16px 0;padding:0 38px;overflow:hidden;border:1px solid #e8dcc9;border-left-width:1px;border-radius:12px;background-color:#fffdf8;background-image:radial-gradient(circle at 1px 1px,rgba(91,72,45,.09) 1px,transparent 1px),radial-gradient(120% 140% at 0 0,rgba(20,75,220,.07),transparent 55%),radial-gradient(120% 140% at 100% 100%,rgba(126,34,206,.09),transparent 55%);background-size:6px 6px,100% 100%,100% 100%;box-shadow:0 10px 30px -18px rgba(76,45,140,.55);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif}.ays-sccp-bts-content{position:relative;z-index:2;display:flex;min-height:110px;align-items:center;justify-content:space-between;gap:28px}.ays-sccp-bts-offer,.ays-sccp-bts-actions{display:flex;align-items:center}.ays-sccp-bts-offer{min-width:0;gap:9px}.ays-sccp-bts-handwritten{width:280px;max-width:23vw;height:auto;color:#0755df;overflow:visible;flex:0 1 auto}.ays-sccp-bts-dot{width:7px;height:7px;flex:0 0 auto;border-radius:50%;background:#ffc51b;align-self:flex-start;margin-top:27px}.ays-sccp-bts-discount{color:#8127df;font-size:38px;font-weight:800;line-height:1;letter-spacing:-1.2px;white-space:nowrap}.ays-sccp-bts-actions{flex:0 0 auto;gap:12px}.ays-sccp-bts-use-code{color:#5f5b69;font-size:11px;font-weight:600;line-height:1;text-transform:uppercase;letter-spacing:.14em;white-space:nowrap}.ays-sccp-bts-coupon{box-sizing:border-box;display:flex;min-height:50px;align-items:center;gap:12px;padding:8px 14px;border:2px dashed rgba(129,39,223,.5);border-radius:10px;background:#fff;color:#1147c8;cursor:pointer;box-shadow:none}.ays-sccp-bts-coupon:hover,.ays-sccp-bts-coupon:focus{border-color:#8127df;background:rgba(129,39,223,.05);color:#1147c8}.ays-sccp-bts-coupon span{font-size:19px;font-weight:700;letter-spacing:.04em}.ays-sccp-bts-coupon svg{width:16px;height:16px;color:#8127df}.ays-sccp-bts-cta{display:inline-flex;min-height:48px;align-items:center;justify-content:center;padding:0 27px;border-radius:9px;background:linear-gradient(90deg,#1249e7,#8127df);color:#fff!important;font-size:16px;font-weight:700;text-decoration:none!important;white-space:nowrap;box-shadow:0 8px 18px -8px rgba(91,37,194,.9);transition:transform .15s,box-shadow .15s}.ays-sccp-bts-cta:hover,.ays-sccp-bts-cta:focus{color:#fff;transform:translateY(-1px);box-shadow:0 10px 20px -8px rgba(91,37,194,.95)}.ays-sccp-bts-decoration{position:absolute;z-index:1;pointer-events:none}.ays-sccp-bts-plane{top:8px;left:12px;width:24px;color:rgba(129,39,223,.55)}.ays-sccp-bts-ruler{bottom:7px;left:24px;width:36px;color:rgba(20,73,230,.35)}.ays-sccp-bts-star{top:8px;right:14px;width:19px;color:#ffc51b}.ays-sccp-bts-pencil{right:31px;bottom:7px;width:21px;color:rgba(129,39,223,.5);transform:rotate(-12deg)}.ays-sccp-bts-wave{bottom:3px;left:50%;width:96px;color:rgba(20,73,230,.22);transform:translateX(-50%)}#ays-sccp-back-to-school-banner-2026 .notice-dismiss{top:1px;right:1px;color:#756b82}#ays-sccp-back-to-school-banner-2026 .notice-dismiss:before{font-size:18px}.ays-sccp-bts-dismiss{position:absolute;right:39px;bottom:0;z-index:3}.ays-sccp-bts-dismiss form{margin:0}.ays-sccp-bts-dismiss #ays-sccp-dismiss-buttons-content .ays-button{height:22px!important;min-height:0!important;margin:0!important;padding:0!important;border:0!important;background:transparent!important;color:#777!important;font-size:10px!important;line-height:22px!important;text-decoration:underline;box-shadow:none!important}.ays-sccp-bts-copy-notification{position:fixed;top:50%;left:50%;z-index:10000;transform:translate(-50%,-50%);padding:12px 24px;border-radius:8px;background:rgba(0,0,0,.82);color:#fff;font-size:14px;opacity:0;transition:opacity .25s}.ays-sccp-bts-copy-notification.ays-sccp-bts-show{opacity:1}@media screen and (max-width:1200px){.ays-sccp-bts-content{gap:18px}.ays-sccp-bts-handwritten{width:225px}.ays-sccp-bts-discount{font-size:31px}.ays-sccp-bts-actions{gap:8px}.ays-sccp-bts-coupon{min-height:44px;padding:7px 10px}.ays-sccp-bts-cta{min-height:44px;padding:0 18px;font-size:14px}}@media screen and (max-width:960px){#ays-sccp-back-to-school-banner-2026{padding:15px 38px 20px}.ays-sccp-bts-content{min-height:90px;flex-wrap:wrap;justify-content:center;gap:12px 24px}.ays-sccp-bts-handwritten{max-width:none}.ays-sccp-bts-actions{justify-content:center}.ays-sccp-bts-dismiss{right:38px}}@media screen and (max-width:600px){#ays-sccp-back-to-school-banner-2026{width:calc(100% - 20px);margin-right:10px;padding:18px 30px 23px 18px}.ays-sccp-bts-content{flex-direction:column}.ays-sccp-bts-offer{flex-wrap:wrap;justify-content:center}.ays-sccp-bts-handwritten{width:210px}.ays-sccp-bts-discount{font-size:28px}.ays-sccp-bts-actions{flex-wrap:wrap}.ays-sccp-bts-use-code{width:100%;text-align:center}.ays-sccp-bts-coupon{min-height:40px}.ays-sccp-bts-coupon span{font-size:16px}.ays-sccp-bts-cta{min-height:40px}.ays-sccp-bts-plane,.ays-sccp-bts-ruler,.ays-sccp-bts-pencil,.ays-sccp-bts-wave{display:none}.ays-sccp-bts-dismiss{right:30px}}';
            $content[] = '</style>';
            $content[] = '<script>
                (function() {
                    "use strict";
                    function initBackToSchoolBanner() {
                        var banner = document.getElementById("ays-sccp-back-to-school-banner-2026");
                        if (!banner) {
                            return;
                        }
                        var couponButton = banner.querySelector(".ays-sccp-bts-coupon");
                        if (!couponButton) {
                            return;
                        }
                    ...
                    function showCopyNotification() {
                        ...
                        notification.className = "ays-sccp-bts-copy-notification";
                        notification.textContent = "'. esc_js(__('Coupon code copied', 'secure-copy-content-protection')) .'";
                        document.body.appendChild(notification);
                        setTimeout(function() {
                            notification.classList.add("ays-sccp-bts-show");
                        }, 10);
                        setTimeout(function() {
                            notification.classList.remove("ays-sccp-bts-show");
                            setTimeout(function() {
    ...
    public static function ays_sccp_footer_sale_banner_2026($ishmar){
        if($ishmar == 0 ){
            $content = array();
            
            $sccp_cta_button_link = esc_url( 'https://ays-pro.com/wordpress/secure-copy-content-protection?utm_source=dashboard&utm_medium=sccp-free&utm_campaign=private-offer-20-off-' . SCCP_NAME_VERSION );

            $content[] = '
                <div class="ays-sccp-footer-banner-bottom-banner-wrapper" style="display: none;">
                    <div class="ays-sccp-footer-banner-bottom-banner-container">
                        <div class="ays-sccp-footer-banner-bottom-banner-content">
                            <div class="ays-sccp-footer-banner-bottom-banner-inner">
                                <div class="ays-sccp-footer-banner-bottom-banner-left">
                                    <div class="ays-sccp-footer-banner-bottom-banner-icon">
                                    ...
                                    <span class="ays-sccp-footer-banner-bottom-banner-title">'. esc_html__('Copy Protection by AYS', 'secure-copy-content-protection') .'</span>
                                </div>
                                <span class="ays-sccp-footer-banner-bottom-banner-separator"></span>
                                <span class="ays-sccp-footer-banner-bottom-banner-discount">'. esc_html__('20% Off', 'secure-copy-content-protection') .'</span>
                                <div class="ays-sccp-footer-banner-bottom-banner-code" onclick="aysSurveyFooterBannerCopyToClipboard()" title="'. esc_html__('Copy Coupon Code', 'secure-copy-content-protection') .'">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    ...
                                    <span>SCCP20OFF</span>
                                    <button type="button" class="ays-sccp-footer-banner-bottom-banner-copy" aria-label="'. esc_html__('Copy Coupon Code', 'secure-copy-content-protection') .'">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                ...
                                <button type="button" class="ays-sccp-footer-banner-bottom-banner-cta">
                                    '. esc_html__('Get Deal', 'secure-copy-content-protection') .'
                                </button>
                                <button type="button" class="ays-sccp-footer-banner-bottom-banner-close" aria-label="'. esc_html__('Close popup', 'secure-copy-content-protection') .'">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            ...
            $content[] = '@keyframes ays-pro-slide-up{0%{opacity:0;transform:translateY(24px)}100%{opacity:1;transform:translateY(0)}}@keyframes ays-pro-slide-down{0%{opacity:1;transform:translateY(0)}100%{opacity:0;transform:translateY(24px)}}.ays-sccp-footer-banner-bottom-banner-wrapper{position:fixed;bottom:0;left:0;right:0;z-index:99999;padding:0 16px 28px;text-align:center;animation:.45s cubic-bezier(.16,1,.3,1) forwards ays-pro-slide-up}.ays-sccp-footer-banner-bottom-banner-wrapper.ays-pro-banner-closing{animation:.32s cubic-bezier(.16,1,.3,1) forwards ays-pro-slide-down}.ays-sccp-footer-banner-bottom-banner-container{display:inline-block;max-width:100%;border-radius:16px;background:linear-gradient(90deg,#2f6bff 0,#3e7bff 100%);box-shadow:0 20px 40px -15px rgba(47,107,255,.55),0 8px 20px -10px rgba(47,107,255,.35);overflow:hidden}.ays-sccp-footer-banner-bottom-banner-code,.ays-sccp-footer-banner-bottom-banner-content,.ays-sccp-footer-banner-bottom-banner-inner,.ays-sccp-footer-banner-bottom-banner-left{display:flex;align-items:center}.ays-sccp-footer-banner-bottom-banner-inner{gap:16px;padding:8px 14px 8px 10px;color:#fff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif}.ays-sccp-footer-banner-bottom-banner-left{gap:14px}.ays-sccp-footer-banner-bottom-banner-icon{display:flex;align-items:center;justify-content:center;flex:0 0 auto;width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,.15);color:#fff}.ays-sccp-footer-banner-bottom-banner-icon svg{width:24px;height:24px}.ays-sccp-footer-banner-bottom-banner-discount,.ays-sccp-footer-banner-bottom-banner-title{font-size:17px;font-weight:600;line-height:1.25;letter-spacing:0;white-space:nowrap;color:#fff}.ays-sccp-footer-banner-bottom-banner-separator{display:block;width:1px;height:24px;background:rgba(255,255,255,.4)}.ays-sccp-footer-banner-bottom-banner-code{gap:6px;border:1px dashed rgba(255,255,255,.7);border-radius:999px;padding:6px 9px;color:#fff;cursor:pointer;transition:background-color .15s,border-color .15s}.ays-sccp-footer-banner-bottom-banner-code:hover{background:rgba(255,255,255,.1);border-color:#fff}.ays-sccp-footer-banner-bottom-banner-code button svg,.ays-sccp-footer-banner-bottom-banner-code>svg{width:16px;height:16px}.ays-sccp-footer-banner-bottom-banner-code span{font-size:14px;font-weight:600;line-height:1;letter-spacing:.05em;color:#fff}.ays-sccp-footer-banner-bottom-banner-close,.ays-sccp-footer-banner-bottom-banner-copy{display:flex;align-items:center;justify-content:center;margin:0;padding:0;background:0 0;border:0;color:rgba(255,255,255,.9);cursor:pointer;box-shadow:none;transition:color .15s,background-color .15s}.ays-sccp-footer-banner-bottom-banner-close:hover,.ays-sccp-footer-banner-bottom-banner-copy:hover{color:#fff;background:0 0}.ays-sccp-footer-banner-bottom-banner-wrapper .ays-sccp-footer-banner-bottom-banner-cta{display:inline-flex;align-items:center;justify-content:center;min-height:40px;padding:8px 20px;border:0;border-radius:999px;background:#fff;color:#2f6bff;font-size:15px;font-weight:600;line-height:1;white-space:nowrap;cursor:pointer;box-shadow:0 1px 2px rgba(0,0,0,.08);transition:background-color .15s,transform .15s}.ays-sccp-footer-banner-bottom-banner-wrapper .ays-sccp-footer-banner-bottom-banner-cta:hover{background:rgba(255,255,255,.95);color:#2f6bff;transform:translateY(-1px)}.ays-sccp-footer-banner-bottom-banner-close{width:22px;height:22px;margin-left:2px}.ays-sccp-footer-banner-bottom-banner-close svg{width:20px;height:20px}.ays-sccp-footer-banner-copy-notification{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,.8);color:#fff;padding:12px 24px;border-radius:8px;font-size:14px;z-index:10000;opacity:0;transition:opacity .3s}.ays-sccp-footer-banner-copy-notification.show{opacity:1}@media (max-width:768px){.ays-sccp-footer-banner-bottom-banner-wrapper{display:none}}';
            $content[] = '</style>';
            ...
            (function() {
              ...
              // Configuration
              var STORAGE_KEY = 'ays_sccp_footer_banner_data';
              var POPUP_RULES = [
              ...
              function initBanner() {
                var banner = document.querySelector('.ays-sccp-footer-banner-bottom-banner-wrapper');
                var closeButton = document.querySelector('.ays-sccp-footer-banner-bottom-banner-close');
                var ctaButton = document.querySelector('.ays-sccp-footer-banner-bottom-banner-cta');
                ...
                if (ctaButton) {
                  ctaButton.addEventListener('click', function() {
                    window.open('https://ays-pro.com/wordpress/secure-copy-content-protection?utm_source=dashboard&utm_medium=sccp-free&utm_campaign=private-offer-20-off-". SCCP_NAME_VERSION ."', '_blank');
            ...
            function aysSurveyFooterBannerCopyToClipboard() {
                ...
                try {
                    document.execCommand('copy');
                    aysSurveyFooterBannerShowCopyNotification('". esc_html__('Coupon code copied', 'secure-copy-content-protection') ."');
                } catch (err) {
            ...
            function aysSurveyFooterBannerShowCopyNotification(message) {
                var existingNotification = document.querySelector('.ays-sccp-footer-banner-copy-notification');
                if (existingNotification) {
                    document.body.removeChild(existingNotification);
                }
                
                var notification = document.createElement('div');
                notification.className = 'ays-sccp-footer-banner-copy-notification';
